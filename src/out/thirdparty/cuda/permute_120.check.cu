...
__global__ void x5(int* x6, int* x7, int x8, int x9, int x10) {
  // this is the permutation kernel for [1, 2, 0]
  ...
  while (x15 < 32) {
    ...
    if (x17 < x8 && blockIdx.x * 32 + (threadIdx.y + x16) < x10) x7[(blockIdx.x * 32 + (threadIdx.y + x16) + blockIdx.y * x10) * x8 + x17] = x11[33 * threadIdx.x + (threadIdx.y + x16)];
    x15 = x15 + 8;
  }
}
/**************** Snippet ****************/
void Snippet(int x0) {
  ...
  scan_int("golden/permute_kernel_120/input.data", x1, 260000);
  ...
  check_int_array("golden/permute_kernel_120/output.data", x3, 260000);
}
/*****************************************
