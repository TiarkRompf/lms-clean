...
void Snippet(int x0) {
  ...
  while (x19 != 10) {
    ...
    // begin computing TANH on GPU for size 512 and type Float at device (pre-rename) x39 with operand x137
    CUDA_CALL(cudaSetDevice(x7));
    ...
    // end computing TANH on GPU for size 512 and type Float at device (pre-rename) x39 with operand x137
