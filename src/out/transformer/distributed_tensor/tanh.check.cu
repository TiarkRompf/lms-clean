...
void Snippet(int x0) {
  ...
  while (x19 != 10) {
    ...
    // begin computing TANH_GRAD on GPU for size 512 and type Float at device (pre-rename) x39 with left_operand x137 and right_operand x65
    CUDA_CALL(cudaSetDevice(x7));
    float* x47 = (float*)malloc(0 * sizeof(float));
    CUDA_CALL(cudaMalloc(&x47, (size_t)(512 * sizeof(float))));
    x27<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x45, x10, x47, 512);
    // end computing TANH_GRAD on GPU for size 512 and type Float at device (pre-rename) x39 with left_operand x137 and right_operand x65
