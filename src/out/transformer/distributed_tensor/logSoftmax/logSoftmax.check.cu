...
void Snippet(int x0) {
  ...
  // begin initializing GPU array of size 17056 and type Float at device (pre-rename) x39 from binary file weight
  ...
  scan_float_rank("golden/weight", x6, x7, 17056);
  CUDA_CALL(cudaMemcpy(x8, x7, (size_t)(17056 * sizeof(float)), cudaMemcpyHostToDevice));
  // end initializing GPU array of size 17056 and type Float at device (pre-rename) x39 from binary file weight
  ...
  // begin initializing GPU array of size 17056 and type Float at device (pre-rename) x39 from binary file input
  ...
  scan_float_rank("golden/input", x6, x16, 17056);
  CUDA_CALL(cudaMemcpy(x17, x16, (size_t)(17056 * sizeof(float)), cudaMemcpyHostToDevice));
  // end initializing GPU array of size 17056 and type Float at device (pre-rename) x39 from binary file input
  CUDA_CALL(cudaSetDevice(x6));
  float* x18 = (float*)malloc(0 * sizeof(float));
  CUDA_CALL(cudaMalloc(&x18, (size_t)(34112 * sizeof(float))));
  x19<<<dim3(64, 1, 1), dim3(1024, 1, 1), 4096>>>(x8, x18, 533);
  ...
  // end initializing fixed GPU array of size 17056 and type Float and device (pre-rename) x39
  // begin checking GPU array of size 17056 and type Float at device (pre-name) x39 again binary file loss
  float* x69 = (float*)malloc(17056 * sizeof(float));
  CUDA_CALL(cudaMemcpy(x69, x58, (size_t)(17056 * sizeof(float)), cudaMemcpyDeviceToHost));
  check_float_array_rank("golden/loss", x6, x69, 17056);
  // end checking GPU array of size 17056 and type Float at device (pre-name) x39 again binary file loss
  // begin initializing fixed GPU array of size 17056 and type Float and device (pre-rename) x39
  CUDA_CALL(cudaSetDevice(x6));
  ...
  check_float_array_rank("golden/weight_grad", x6, x101, 17056);
  // end checking GPU array of size 17056 and type Float at device (pre-name) x39 again binary file weight_grad
  // begin checking GPU array of size 17056 and type Float at device (pre-name) x39 again binary file input_grad
  float* x102 = (float*)malloc(17056 * sizeof(float));
  CUDA_CALL(cudaMemcpy(x102, x67, (size_t)(17056 * sizeof(float)), cudaMemcpyDeviceToHost));
  check_float_array_rank("golden/input_grad", x6, x102, 17056);
  // end checking GPU array of size 17056 and type Float at device (pre-name) x39 again binary file input_grad
