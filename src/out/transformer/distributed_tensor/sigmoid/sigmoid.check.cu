...
void Snippet(int x0) {
  ...
  CUDNNCHECK(cudnnSetActivationDescriptor(x20, CUDNN_ACTIVATION_SIGMOID, CUDNN_PROPAGATE_NAN, 0.0));
  ...
  CUDA_CALL(cudaMalloc(&x21, (size_t)(18 * sizeof(float))));
  ...
  check_float_array_rank("golden/loss", x6, (float*)malloc(9 * sizeof(float)), x35, 9);
  ...
  // end initializing fixed GPU array of size 9 and type Float and device (pre-rename) x39
  // begin computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x216 and addition_operand x240
  CUDA_CALL(cudaSetDevice(x6));
  x37<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x34, x36, 9);
  // end computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x216 and addition_operand x240
  // begin computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x203 and addition_operand x240
  CUDA_CALL(cudaSetDevice(x6));
  x37<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x33, x36, 9);
  // end computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x203 and addition_operand x240
  ...
  // begin activation backward pass
  // begin computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x67 and addition_operand x297
  CUDA_CALL(cudaSetDevice(x6));
  x37<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x10, x44, 9);
  // end computing ACCUM on GPU for size 9 and type Float at device (pre-rename) x39 with base_operand x67 and addition_operand x297
  ...
  check_float_array_rank("golden/weight_grad", x6, (float*)malloc(9 * sizeof(float)), x47, 9);
  ...
  check_float_array_rank("golden/input_grad", x6, (float*)malloc(9 * sizeof(float)), x48, 9);
