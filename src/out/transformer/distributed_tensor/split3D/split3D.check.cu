...
void Snippet(int x0) {
  ...
  // begin initializing GPU array of size 8192 and type Float at device (pre-rename) x39 from binary file weight
  ...
  scan_float_rank("golden/weight", x6, x7, 8192);
  CUDA_CALL(cudaMemcpy(x8, x7, (size_t)(8192 * sizeof(float)), cudaMemcpyHostToDevice));
  // end initializing GPU array of size 8192 and type Float at device (pre-rename) x39 from binary file weight
  ...
  // begin initializing GPU array of size 16384 and type Float at device (pre-rename) x39 from binary file input
  ...
  scan_float_rank("golden/input", x6, x16, 16384);
  CUDA_CALL(cudaMemcpy(x17, x16, (size_t)(16384 * sizeof(float)), cudaMemcpyHostToDevice));
  // end initializing GPU array of size 16384 and type Float at device (pre-rename) x39 from binary file input
  CUDA_CALL(cudaSetDevice(x6));
  ...
  x22<<<dim3(64, 1, 1), dim3(512, 1, 1)>>>(x17, x21);
  ...
  // begin checking GPU array of size 8192 and type Float at device (pre-name) x39 again binary file weight_grad
  float* x59 = (float*)malloc(8192 * sizeof(float));
  CUDA_CALL(cudaMemcpy(x59, x9, (size_t)(8192 * sizeof(float)), cudaMemcpyDeviceToHost));
  check_float_array_rank("golden/weight_grad", x6, x59, 8192);
  // end checking GPU array of size 8192 and type Float at device (pre-name) x39 again binary file weight_grad
  // begin checking GPU array of size 16384 and type Float at device (pre-name) x39 again binary file input_grad
  float* x60 = (float*)malloc(16384 * sizeof(float));
  CUDA_CALL(cudaMemcpy(x60, x37, (size_t)(16384 * sizeof(float)), cudaMemcpyDeviceToHost));
  check_float_array_rank("golden/input_grad", x6, x60, 16384);
  // end checking GPU array of size 16384 and type Float at device (pre-name) x39 again binary file input_grad
