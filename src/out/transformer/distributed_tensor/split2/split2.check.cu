...
__global__ void x56(float* x57, float* x58, float* x59, int x60, int x61) {
  ...
  while (x64 < x63) {
    int x66 = x64;
    int x67 = x66 % x65;
    if (x67 < x61) x59[x66] = x57[x66 / x65 * x61 + x67];
    else x59[x66] = x58[x66 / x65 * x61 + x67 - x61];
    x64 = x64 + x62;
  }
...
void Snippet(int x0) {
  ...
  // begin initializing GPU array of size 256 and type Float
  ...
  scan_float_array(x16, 256, "golden/input_rank_%d.data", x6);
  CUDA_CALL(cudaMemcpy(x17, x16, (size_t)(256 * sizeof(float)), cudaMemcpyHostToDevice));
  // end initializing GPU array of size 256 and type Float
  ...
  check_float_array_with_file(x44, 256, "golden/loss_rank_%d.data", x6);
  ...
  check_float_array_with_file(x68, 512, "golden/weight_grad_rank_%d.data", x6);
  // end checking GPU array of size 512 and type Float
  // begin checking GPU array of size 256 and type Float
  float* x69 = (float*)malloc(256 * sizeof(float));
  CUDA_CALL(cudaMemcpy(x69, x41, (size_t)(256 * sizeof(float)), cudaMemcpyDeviceToHost));
  check_float_array_with_file(x69, 256, "golden/input_grad_rank_%d.data", x6);
  // end checking GPU array of size 256 and type Float
