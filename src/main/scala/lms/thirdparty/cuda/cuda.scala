...
object CUDATypeLess extends Dsl with StackArrayOps with CLibs with CudaFunction {
  ...
  def CUDA_TANH_GRAD_KERNEL(m: Manifest[_])(implicit __pos: SourceContext) = CUDA_ELEMENTWISE_BINARY_KERNEL(m, _ tanh_grad(_, m), s"generating kernel function for TANH_GRAD of type $m")
  ...
  def CUDA_RELU_GRAD_KERNEL(m: Manifest[_])(implicit __pos: SourceContext) = CUDA_UNARY_KERNEL(m, _ relu_grad(m), s"generating kernel function for RELU_GRAD of type $m")
