...
void RunTest(T **sendbuff, T **recvbuff, const int N, const ncclDataType_t type,
             const ncclRedOp_t op, ncclComm_t *comms,
  ...
  for (int i = 0; i < nDev; ++i) {
    ...
    NCCLCHECK(ncclAllReduce((const void *)sendbuff[i], (void *)recvbuff[i],
                            std::min(N, 1024 * 1024), type, op, comms[i],
                            s[i]));
  }
  ...
  //  for (int n = 0; n <= N; n = (n > 0) ? n << 1 : 1)
  {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(dList[i]));
      NCCLCHECK(ncclAllReduce((const void *)sendbuff[i], (void *)recvbuff[i], n,
                              type, op, comms[i], s[i]));
    }
    //}
  ...
  //  for (int n = 0; n <= N; n = (n > 0) ? n << 1 : 1)
  {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(dList[i]));
      NCCLCHECK(ncclAllReduce((const void *)sendbuff[i], (void *)sendbuff[i], n,
                              type, op, comms[i], s[i]));
    }
    //}
...
int main(int argc, char *argv[]) {
  ...
  if (!csv) {
    ...
    printf("# %10s  %12s  %6s  %6s  %7s  %5s  %5s  %7s  %7s  %5s  %5s  %7s\n",
           "bytes", "N", "type", "op", "time", "algbw", "busbw", "res", "time",
           "algbw", "busbw", "res");
  } else {
    printf("B,N,type,op,oop_time,oop_algbw,oop_busbw,oop_res,ip_time,ip_algbw,"
           "ip_busbw,ip_res\n");
  }
