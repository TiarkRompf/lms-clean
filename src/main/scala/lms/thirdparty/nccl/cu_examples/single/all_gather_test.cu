...
void RunTest(T **sendbuff, T **recvbuff, const int N, const ncclDataType_t type,
  ...
  for (int i = 0; i < nDev; ++i) {
    CUDACHECK(cudaSetDevice(dList[i]));
    NCCLCHECK(ncclAllGather((const void *)sendbuff[i], std::min(32 * 1024, N),
                            type, (void *)recvbuff[i], comms[i], s[i]));
  }
  ...
  // for (int n = 1; n <= N; n = n << 1)
  {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(dList[i]));
      NCCLCHECK(ncclAllGather((const void *)sendbuff[i], n, type,
    ...
    double elapsedSec =
        std::chrono::duration_cast<std::chrono::duration<double>>(stop - start)
            .count();
    double algbw =
        (double)(n * sizeof(T)) / 1.0E9 * (double)(nDev - 1) / elapsedSec;
    double busbw = algbw;
