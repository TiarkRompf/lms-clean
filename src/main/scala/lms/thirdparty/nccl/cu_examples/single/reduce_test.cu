...
void RunTest(T **sendbuff, T **recvbuff, const int N, const ncclDataType_t type,
  ...
  for (int i = 0; i < nDev; ++i) {
    ...
    if (i == 0) {
      CUDACHECK(cudaMemcpy(result, sendbuff[i], N * sizeof(T),
  ...
  for (int i = 0; i < nDev; ++i) {
    CUDACHECK(cudaSetDevice(dList[i]));
    NCCLCHECK(ncclReduce((const void *)sendbuff[i], (void *)recvbuff[i],
                         std::min(N, 1024 * 1024), type, op, root, comms[i],
                         s[i]));
  }
  ...
  //  for (int n = 0; n <= N; n = (n > 0) ? n << 1 : 1)
  {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(dList[i]));
      NCCLCHECK(ncclReduce((const void *)sendbuff[i], (void *)recvbuff[i], n,
    ...
    if (maxDelta > deltaMaxValue(type, is_reduction))
      errors++;
    avg_bw += busbw;
    avg_count++;
  ...
  //  for (int n = 0; n <= N; n = (n > 0) ? n << 1 : 1)
  {
    ...
    for (int i = 0; i < nDev; ++i) {
      CUDACHECK(cudaSetDevice(dList[i]));
      NCCLCHECK(ncclReduce((const void *)sendbuff[i], (void *)sendbuff[i], n,
                           type, op, root, comms[i], s[i]));
    }
    //}
...
int main(int argc, char *argv[]) {
  ...
  if (!csv) {
    ...
    printf("# %10s  %12s  %6s  %6s  %4s        out-of-place                    "
           "in-place\n",
           "", "", "", "", "");
    printf(
        "# %10s  %12s  %6s  %6s  %4s %7s  %5s  %5s  %7s  %7s  %5s  %5s  %7s\n",
        "bytes", "N", "type", "op", "root", "time", "algbw", "busbw", "res",
        "time", "algbw", "busbw", "res");
  } else {
    printf("B,N,type,op,root,oop_time,oop_algbw,oop_busbw,oop_res,ip_time,ip_"
           "algbw,ip_busbw,ip_res\n");
  }
  ...
  RunTests<unsigned long long>(N / sizeof(unsigned long long), ncclUint64,
                               comms, dList);
  ...
  printf(" Avg bus bandwidth    : %g %s\n", avg_bw,
         check_avg_bw == -1 ? "" : (avg_bw < check_avg_bw ? "FAILED" : "OK"));
  printf("\n");
