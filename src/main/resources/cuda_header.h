...
template <typename T>
__global__ void hardTanh_grad(T* in_x, T* in_d, T* out_d, T min_val, T max_val, int size, bool inplace) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = gridDim.x * blockDim.x;
  for (int i = tid; i < size; i += stride) {
    if (inplace) {
...
template <typename T>
__global__ void sum_grad(T* in, int inSize0, int inSize1, int inSize2, int inSize3, int nElement,
                         T* out, int outStride0, int outStride1, int outStride2, int dim) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = gridDim.x * blockDim.x;
